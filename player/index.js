...
let tryCount = 0;
const audios = [
	"./player/audio/example-1.mp3",
	"./player/audio/example-2.mp3",
	"./player/audio/example-3.mp3",
];
...
function play(clearTime = false) {
    ...
    if(!audio.duration){
        if(tryCount >= audios.length) return pause();
        tryCount++;
        next();
        return;
    }
...
function next() {
	if (currentSong + 1 <= audios.length - 1) currentSong++;
	else currentSong = 0;
	play(true);
}
...
function prev() {
	...
	else currentSong = audios.length - 1;
	play(true);
}
...
function createNewAudioLayout() {
	...
	image.src = "./player/images/snowflake.png";
	...
	[{ name: "prev" }, { name: "play" }, { name: "next" }].forEach((value) => {
		...
		if (value.name === "play") btn.classList.add("paused");
